...
bool     RADIO_ValidateVfo(VFO_Info_t *pInfo);
...
bool RADIO_ValidateVfo(VFO_Info_t *pInfo)
{
    ...
    if (pInfo->STEP_SETTING >= STEP_N_ELEM) {
        pInfo->STEP_SETTING = STEP_12_5kHz;
    }
    pInfo->StepFrequency = gStepFrequencyTable[pInfo->STEP_SETTING];

    if (pInfo->freq_config_RX.CodeType >= 16) { 
        pInfo->freq_config_RX.CodeType = CODE_TYPE_OFF;
    }
    if (pInfo->freq_config_TX.CodeType >= 16) {
        pInfo->freq_config_TX.CodeType = CODE_TYPE_OFF;
    }
    ...
    if (pInfo->CHANNEL_BANDWIDTH > 1) {
        pInfo->CHANNEL_BANDWIDTH = 0; // Wide
    }

    if (pInfo->TX_OFFSET_FREQUENCY_DIRECTION > TX_OFFSET_FREQUENCY_DIRECTION_SUB) {
        pInfo->TX_OFFSET_FREQUENCY_DIRECTION = TX_OFFSET_FREQUENCY_DIRECTION_OFF;
    }

    if (pInfo->TX_OFFSET_FREQUENCY >= _1GHz_in_KHz) {
        pInfo->TX_OFFSET_FREQUENCY = 0;
    }

    // Additional validations for Phase 2
    pInfo->FrequencyReverse  = !!pInfo->FrequencyReverse;
    pInfo->BUSY_CHANNEL_LOCK = !!pInfo->BUSY_CHANNEL_LOCK;
    pInfo->TX_LOCK           = !!pInfo->TX_LOCK;

#ifdef ENABLE_DTMF_CALLING
    pInfo->DTMF_DECODING_ENABLE = !!pInfo->DTMF_DECODING_ENABLE;
#endif
    if (pInfo->DTMF_PTT_ID_TX_MODE > PTT_ID_BOTH) {
        pInfo->DTMF_PTT_ID_TX_MODE = PTT_ID_OFF;
    }

    if (pInfo->SCRAMBLING_TYPE > 10) { // arbitrary limit based on menu/enum
        pInfo->SCRAMBLING_TYPE = 0;
    }

    if (!IS_VALID_CHANNEL(pInfo->CHANNEL_SAVE)) {
        pInfo->CHANNEL_SAVE = FREQ_CHANNEL_FIRST + BAND3_137MHz;
    }
...
void RADIO_ConfigureChannel(const unsigned int VFO, const unsigned int configure)
{
    ...
    if (configure == VFO_CONFIGURE_RELOAD || IS_FREQ_CHANNEL(channel))
    {
        ...
        // ***************
        if (!RADIO_ValidateVfo(pVfo) && IS_MR_CHANNEL(channel)) {
            // Memory channel corrupted (illegal frequency) - mark as empty
            gMR_ChannelAttributes[channel].__val = 0xFF;
        }
    }
